        ...
        public void Update()
        {
            ...
            if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject())
            {
                if (Physics.Raycast(_camera.ScreenPointToRay(Input.mousePosition), out RaycastHit hit))
                {
                    ...
                    float stoppingDistance = _attackSettings.DefaultStopDistance;
                    IInteractable currentTarget = null;

                    if (clickedObject.TryGetComponent(out currentTarget))
                    {
                        if (currentTarget is Enemy)
                        {
                            stoppingDistance = _attackSettings.CurrentRange;
                    ...
                    // Set the calculated stopping distance
                    _agent.stoppingDistance = stoppingDistance;

                    // Move the player to the calculated destination
